...
export class SymlinkManager {
  ...
  public enableCapability(canonicalPath: string): boolean {
  ...
  public disableCapability(canonicalPath: string): boolean {
    const disabledPath = `${canonicalPath}.asg-disabled`;
  ...
  public syncState(
    capabilities: Array<{ id: string; canonicalFilePath: string }>,
    activeIds: string[]
  ): void {
  ...
  public restoreAll(capabilities: Array<{ canonicalFilePath: string }>): void {
    for (const cap of capabilities) {
      this.enableCapability(cap.canonicalFilePath);
    }
  }
